...
def main():
    # Load data
    print("Loading data...")
    data = pd.read_parquet('data/Features_training_BMIQ.parquet')
    ...
    print(loco_results)
    loco_results.to_csv('LOCO_results_BMIQ.csv', index=False)
    print("Saved LOCO results to LOCO_results_BMIQ.csv")
    
    loco_predictions.to_csv('LOCO_predictions_BMIQ.csv', index=False)
    print(f"Saved {len(loco_predictions)} predictions to LOCO_predictions_BMIQ.csv")
    ...
    print(f"Saving {var_clock.shape[0] - 1} non-zero coefficients + intercept to BTEC_BMIQ.csv")
    var_clock.to_csv('BTEC_BMIQ.csv')
